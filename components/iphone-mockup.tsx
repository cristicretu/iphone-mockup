...
export function IphoneMockup() {
  ...
  const createSnapshot = useCallback(() => {
    if (!mockupRef.current || !videoRef.current || !canvasRef.current) return null
    ...
    ctx.drawImage(videoRef.current, screenX, screenY, screenWidth, screenHeight)
    ctx.restore()
    ...
    <div className="grid md:grid-cols-2 gap-8">
      <Card className="p-6 flex flex-col gap-4">
        ...
        <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center">
          <input type="file" accept="video/*" id="video-upload" className="hidden" onChange={handleFileChange} />
          <label htmlFor="video-upload" className="flex flex-col items-center gap-3 cursor-pointer">
            <Upload className="h-10 w-10 text-slate-400" />
            <span className="text-slate-600">{videoFile ? videoFile.name : "Click to upload your video"}</span>
          </label>
        </div>
        ...
        {videoUrl && (
          <div className="mt-4">
            ...
            <p className="text-sm text-slate-500 mb-3">
              This will capture the current frame of your video in the iPhone mockup.
            </p>
            <Button onClick={downloadFrame} disabled={!videoUrl || isProcessing}>
      ...
      <div>
        <div ref={mockupRef} className="relative mx-auto" style={{ width: "300px", height: "600px" }}>
          {/* iPhone mockup */}
          <div className="absolute inset-0 bg-black rounded-[3rem] shadow-xl overflow-hidden border-[14px] border-black">
            ...
            {/* iPhone screen */}
            <div className="relative h-full w-full bg-slate-800 overflow-hidden rounded-[2rem]">
              {videoUrl ? (
                <div className="absolute inset-0 overflow-hidden" style={{ borderRadius: "1.8rem" }}>
                  <video
                    ref={videoRef}
                    src={videoUrl}
